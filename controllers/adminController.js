...
exports.getServices = async (req, res) => {
  try {
    ...
    res.render('admin/services', { title: 'Manage Services', services, providers });
  } catch (err) {
  ...
  try {
    ...
    for (const ps of providerServices) {
      ...
      await Service.create({
        ...
        sellPricePer1000: Math.ceil(cost * 1.3 * 100) / 100, // default 30% markup, admin can edit
...
exports.updateSettings = async (req, res) => {
  try {
    const { siteName, easypaisaNumber, easypaisaAccountName, currency, whatsappNumber, minimumDeposit } = req.body;
