...
exports.importServicesFromProvider = async (req, res) => {
  try {
    ...
    for (const ps of providerServices) {
      ...
      // FIX: Convert provider USD rate to PKR before saving
      ...
      await Service.create({
        ...
        sellPricePer1000: Math.ceil(costInPkr * markupMultiplier * 100) / 100, // your commission %, admin can edit
...
exports.applyCommissionToServices = async (req, res) => {
  try {
    ...
    // Fetch fresh rates from the provider API to ensure the base cost is accurate
    ...
    const rateMap = new Map(); // key: providerServiceId, value: rate (in PKR)
    for (const [id, provider] of providerMap) {
      try {
        const providerServices = await providerApi.getServices({ apiUrl: provider.apiUrl, apiKey: provider.apiKey });
        providerServices.forEach(ps => {
          // FIX: Convert provider USD rate to PKR
          const rawRate = parseFloat(ps.rate) || 0;
